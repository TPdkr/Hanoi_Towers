...
fun output(){
    val sidemaxwidth=towers.height//how far from the tower center can we go
    for(i in 0 until towers.height){//for each layer
        val newline=StringBuilder()
        for(j in 0 until 3){
            newline.append(disktoline(towers.twrs[j].disks[i],sidemaxwidth))
        }
